#include "ChatPersistentMessageToServerCallback.h"
...
ChatPersistentMessageToServerCallback::ChatPersistentMessageToServerCallback(ZoneClientSession* client, ZoneProcessServer* server) : MessageCallback(client, server) {
	sequence = 0;

	setCustomTaskQueue("slowQueue");
}

void ChatPersistentMessageToServerCallback::parse(Message* message) {
...
int ChatPersistentMessageToServerCallback::sendMail(CreatureObject* player, const String& recipient) {
...
int ChatPersistentMessageToServerCallback::sendMailToPlayer(CreatureObject* player, const String& recipientName) {
	auto chatManager = server->getChatManager();
	auto playerManager = server->getZoneServer()->getPlayerManager();
...
void ChatPersistentMessageToServerCallback::run() {
