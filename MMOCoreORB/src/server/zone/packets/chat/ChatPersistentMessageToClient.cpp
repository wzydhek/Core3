#include "ChatPersistentMessageToClient.h"
#include "server/chat/WaypointChatParameterVector.h"

ChatPersistentMessageToClient::ChatPersistentMessageToClient(const String& sender, uint32 mailid, uint8 type, const UnicodeString& subject, const UnicodeString& body, uint32 timestamp, char status) : BaseMessage() {
	insertShort(0x02);
...
ChatPersistentMessageToClient::ChatPersistentMessageToClient(const String& sender, uint32 mailid, uint8 type, const UnicodeString& subject, StringIdChatParameter& body, uint32 timestamp, char status) : BaseMessage() {
	insertShort(0x02);
	insertInt(0x08485E17); // CRC
...
ChatPersistentMessageToClient::ChatPersistentMessageToClient(PersistentMessage* mail, const String& serverName, bool sendBody) {
	insertShort(0x02);
...
void ChatPersistentMessageToClient::insertParameters(PersistentMessage* mail) {
