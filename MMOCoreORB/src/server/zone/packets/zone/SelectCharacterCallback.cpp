#include "SelectCharacterCallback.h"
...
SelectCharacterCallback::SelectCharacterCallback(ZoneClientSession* client, ZoneProcessServer* server) : MessageCallback(client, server), characterID(0) {
	setCustomTaskQueue("slowQueue");
}

void SelectCharacterCallback::parse(Message* message) {
	characterID = message->parseLong(6);
}

void SelectCharacterCallback::connectPlayer(SceneObject* obj, uint64_t characterID, CreatureObject* player, ZoneClientSession* client, ZoneServer* zoneServer) {
	...
	SWGRealmsAPI::instance()->approvePlayerConnect(clientIP, ghost->getAccountID(), characterID, loggedInAccounts, [object = Reference<SceneObject*>(obj), characterID, playerCreature = Reference<CreatureObject*>(player), clientObject = Reference<ZoneClientSession*>(client), zoneServer](const SessionApprovalResult& result) {
...
void SelectCharacterCallback::connectApprovedPlayer(SceneObject* obj, uint64_t characterID, CreatureObject* player, ZoneClientSession* client, ZoneServer* zoneServer) {
	...
	debugMsg << endl << endl << "=============================================" << endl << "---------- SelectCharacterCallback ----------" << endl << "Player: " << player->getDisplayedName() << endl << "Zone: " << zoneName << endl;
	...
	*/
	...
	if (!currentParentNull && playerParent != nullptr && ((!unloadedInParent && currentParent->isValidJtlParent()) || (currentParent->isCellObject() && rootParent != nullptr && rootParent->isPobShip() && rootParent->getLocalZone() != nullptr))) {
#ifdef DEBUG_SELECT_CHAR_CALLBACK
...
void SelectCharacterCallback::run() {
	...
	if (obj != nullptr && obj->isPlayerCreature()) {
		...
		StringBuffer reason;
		if (obj != nullptr) {
			reason << "object " << String::hexvalueOf((int64)characterID) << " loaded but is not a player creature (gameObjectType=" << String::hexvalueOf((int64)obj->getGameObjectType()) << ")";
		} else {
			reason << "object " << String::hexvalueOf((int64)characterID) << " not found in object broker (account=" << client->getAccountID() << " galaxy=" << zoneServer->getGalaxyID() << ")";
		}

		client->error() << "SelectCharacter failed: " << reason.toString();
		Logger::console.error() << "SelectCharacter failed for account " << client->getAccountID() << ": " << reason.toString();

		ErrorMessage* errMsg = new ErrorMessage("Login Error", "Server could not load this character.\nContact support if this persists.", 0x0);
		client->sendMessage(errMsg);
	}
}