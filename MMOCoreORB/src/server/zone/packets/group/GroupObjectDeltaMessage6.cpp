#include "GroupObjectDeltaMessage6.h"
#include "server/zone/objects/creature/CreatureObject.h"

GroupObjectDeltaMessage6::GroupObjectDeltaMessage6(GroupObject* gr) : DeltaMessage(gr->getObjectID(), 'GRUP', 0x06) {
	// info(true) << "GroupObjectDeltaMessage6 called";

	group = gr;
}

void GroupObjectDeltaMessage6::initialUpdate() {
...
void GroupObjectDeltaMessage6::updateMembers() {
...
void GroupObjectDeltaMessage6::updateLevel(uint16 value) {
	startUpdate(0x04);
	insertShort(value);
}

void GroupObjectDeltaMessage6::updateLootRules(uint64 looter, int rule) {
	startUpdate(0x06);
	insertLong(looter);
	startUpdate(0x07);
	insertInt(rule);
