...
CreateProjectileMessage::CreateProjectileMessage(Vector3 position, Vector3 direction, uint8 component, uint8 projectile, uint8 weapon, uint16 shipID, uint32 sequence) {
	insertShort(0x12);
...
CreateProjectileMessage::CreateProjectileMessage(ShipObject* ship, const ShipProjectile* projectile, uint32 syncStamp) {
	insertShort(0x12);
	insertInt(0xB88AF9A5);
...
CreateProjectileMessageCallback::CreateProjectileMessageCallback(ZoneClientSession* client, ZoneProcessServer* server) : MessageCallback(client, server) {
	shipID = 0;
	weaponIndex = 0;
	projectileType = 0;
	componentIndex = 0;
...
void CreateProjectileMessageCallback::parse(Message* message) {
...
void CreateProjectileMessageCallback::run() {
...
void CreateProjectileMessageCallback::launchCountermeasure(ShipObject* ship, CreatureObject* pilot, const ShipProjectileData* data) const {
	auto shipManager = ShipManager::instance();
...
void CreateProjectileMessageCallback::launchMissile(ShipObject* ship, CreatureObject* pilot, const ShipProjectileData* data) const {
...
const char* CreateProjectileMessageCallback::getTaskName() {
