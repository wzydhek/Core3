#include "PlanetTravelPointListRequestCallback.h"
#include "server/zone/managers/planet/PlanetManager.h"
#include "PlanetTravelPointListResponse.h"
#include "server/zone/Zone.h"

PlanetTravelPointListRequestCallback::PlanetTravelPointListRequestCallback(ZoneClientSession* client, ZoneProcessServer* server) : MessageCallback(client, server) {
}

void PlanetTravelPointListRequestCallback::parse(Message* message) {
	message->shiftOffset(8); // We don't need the player object, we got it already.
	message->parseAscii(zoneName);
}

void PlanetTravelPointListRequestCallback::run() {
