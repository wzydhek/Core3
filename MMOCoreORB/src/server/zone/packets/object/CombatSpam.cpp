#include "CombatSpam.h"

CombatSpam::CombatSpam(TangibleObject* attacker, TangibleObject* defender, CreatureObject* receiver, TangibleObject* item, uint32 damage, const String& file, const String& stringName, byte color) : StandaloneObjectControllerMessage(receiver->getObjectID(), 0x1B, 0x134) {
	if (attacker != nullptr)
...
CombatSpam::CombatSpam(CreatureObject* receiver, const UnicodeString& uniString, byte color) : StandaloneObjectControllerMessage(receiver->getObjectID(), 0x1B, 0x134) {
	insertLong(0);
	insertLong(0);
	insertLong(0);
	insertInt(0);
