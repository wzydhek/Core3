#include "DataTransformWithParent.h"
...
DataTransformWithParent::DataTransformWithParent(SceneObject* sceneO) : ObjectControllerMessage(sceneO->getObjectID(), 0x1B, 0xF1) {
...
DataTransformWithParentCallback::DataTransformWithParentCallback(ObjectControllerMessageCallback* objectControllerCallback) : MessageCallback(objectControllerCallback->getClient(), objectControllerCallback->getServer()) {
...
void DataTransformWithParentCallback::parse(Message* message) {
	transform.parseDataTransformWithParent(message);

	debug() << "DataTransformWithParent parsed.";
}

void DataTransformWithParentCallback::updateError(CreatureObject* creO, const String& message, bool bounceBack) const {
...
void DataTransformWithParentCallback::run() {
	...
	try {
		if (playerPosition.getParent() != transform.getParentID() || (!spaceZone && transform.get2dSquaredDistance(playerPosition.getPosition()) > Transform::UPDATE_THRESHOLD) || (spaceZone && transform.get3dSquaredDistance(playerPosition.getPosition()) > Transform::UPDATE_THRESHOLD)) {
...
void DataTransformWithParentCallback::updatePosition(CreatureObject* creO, SceneObject* parent) {
...
void DataTransformWithParentCallback::updateStatic(CreatureObject* creO, SceneObject* parent) {
...
void DataTransformWithParentCallback::broadcastTransform(CreatureObject* creO, SceneObject* parent, const Vector3& position, bool lightUpdate) const {
	auto ghost = creO->getPlayerObject();
