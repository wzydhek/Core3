#include "LotteryWindowCallback.h"
#include "server/zone/ZoneServer.h"
#include "server/zone/objects/player/sessions/LootLotterySession.h"

LotteryWindowCallback::LotteryWindowCallback(ObjectControllerMessageCallback* objectControllerCallback) : MessageCallback(objectControllerCallback->getClient(), objectControllerCallback->getServer()), containerID(0), listSize(0), objectControllerMain(objectControllerCallback) {
}

void LotteryWindowCallback::parse(Message* msg) {
	msg->parseInt();
...
void LotteryWindowCallback::run() {
