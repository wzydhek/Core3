...
ComponentSlot::ComponentSlot() : IngredientSlot() {
	clientSlotType = 2;
	setLoggingName("ComponentSlot");
}

ComponentSlot::ComponentSlot(const ComponentSlot& slot) : Object(), IngredientSlot(slot) {
...
Object* ComponentSlot::clone() {
	return new ComponentSlot(*this);
}

bool ComponentSlot::add(CreatureObject* player, SceneObject* satchel, ManagedReference<TangibleObject*> incomingTano) {
...
bool ComponentSlot::returnToParents(CreatureObject* player) {
...
int ComponentSlot::getSlotQuantity() {
	int quantity = 0;
	for (int i = 0; i < contents.size(); ++i) {
...
bool ComponentSlot::isFull() {
	return requiredQuantity == getSlotQuantity();
}

bool ComponentSlot::isEmpty() {
	return getSlotQuantity() == 0;
}

TangibleObject* ComponentSlot::getPrototype() {
	if (contents.isEmpty())
		return nullptr;
...
SceneObject* ComponentSlot::getFactoryIngredient() {
	return getPrototype();
}

bool ComponentSlot::isComponentSlot() {
	return true;
}

Vector<uint64> ComponentSlot::getOIDVector() {
	Vector<uint64> oids;
...
Vector<int> ComponentSlot::getQuantityVector() {
	Vector<int> quant;
	for (int i = 0; i < getSlotQuantity(); ++i) {
		quant.add(1);
	}
