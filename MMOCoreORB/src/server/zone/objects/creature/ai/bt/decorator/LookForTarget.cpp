#include "LookForTarget.h"
#include "server/zone/managers/creature/observers/CreatureHerdObserver.h"

LookForTarget::LookForTarget(const String& className, const uint32 id, const LuaObject& args) : Decorator(className, id, args) {
}

LookForTarget::LookForTarget(const LookForTarget& b) : Decorator(b) {
}

Behavior::Status LookForTarget::execute(AiAgent* agent, unsigned int) const {
#ifdef DEBUG_AI
...
bool LookForTarget::isInvalidTarget(CreatureObject* target, AiAgent* agent) const {
