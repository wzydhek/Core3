#include "LookForAllies.h"

LookForAllies::LookForAllies(const String& className, const uint32 id, const LuaObject& args) : Decorator(className, id, args) {
}

LookForAllies::LookForAllies(const LookForAllies& b) : Decorator(b) {
}

LookForAllies& LookForAllies::operator=(const LookForAllies& b) {
	if (this == &b)
		return *this;
	Behavior::operator=(b);
	return *this;
...
Behavior::Status LookForAllies::execute(AiAgent* agent, unsigned int startIdx) const {
...
bool LookForAllies::isInvalidTarget(CreatureObject* target, AiAgent* agent) const {
	if (target == nullptr || target->isDead() || target == agent || !target->isAiAgent() || !(target->getPvpStatusBitmask() & ObjectFlag::ATTACKABLE) || !(target->getOptionsBitmask() & OptionBitmask::AIENABLED) || target->isInCombat())
		return true;
