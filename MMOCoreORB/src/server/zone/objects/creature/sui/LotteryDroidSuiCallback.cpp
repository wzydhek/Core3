#include "LotteryDroidSuiCallback.h"

LotteryDroidSuiCallback::LotteryDroidSuiCallback(ZoneServer* server, int optArg) : SuiCallback(server) {
	optionalArg = optArg;
}

void LotteryDroidSuiCallback::run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
