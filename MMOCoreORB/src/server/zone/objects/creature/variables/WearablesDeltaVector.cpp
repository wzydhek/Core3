#include "WearablesDeltaVector.h"

WearablesDeltaVector::WearablesDeltaVector() : DeltaVector<ManagedReference<TangibleObject*>>() {
	protectionArmorMap.setAllowOverwriteInsertPlan();

	// addSerializableVariable("protectionArmorMap", &protectionArmorMap);
}

bool WearablesDeltaVector::readObjectMember(ObjectInputStream* stream, const String& name) {
...
int WearablesDeltaVector::writeObjectMembers(ObjectOutputStream* stream) {
...
bool WearablesDeltaVector::toBinaryStream(ObjectOutputStream* stream) {
...
bool WearablesDeltaVector::parseFromBinaryStream(ObjectInputStream* stream) {
...
void WearablesDeltaVector::insertItemToMessage(ManagedReference<TangibleObject*>* item, BaseMessage* msg) const {
...
bool WearablesDeltaVector::add(const ManagedReference<TangibleObject*>& element, DeltaMessage* message, int updates) {
...
ManagedReference<TangibleObject*> WearablesDeltaVector::remove(int index, DeltaMessage* message, int updates) {
...
Vector<ManagedReference<ArmorObject*>> WearablesDeltaVector::getArmorAtHitLocation(uint8 hitLocation) const {
...
void WearablesDeltaVector::addArmor(uint8 hitLocation, ManagedReference<ArmorObject*> armor) {
...
void WearablesDeltaVector::removeArmor(uint8 hitLocation, ManagedReference<ArmorObject*> armor) {
	Vector<ManagedReference<ArmorObject*>> armors = protectionArmorMap.get(hitLocation);
