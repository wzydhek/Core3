...
int ServerLootCommand::executeCommand(CreatureObject* creature, uint64 target, const UnicodeString& arguments) {
	...
	systemMessage << "ServerLootCommand:" << endl << endl;

	if (!command.isEmpty()) {
		systemMessage << "--------------------------------" << endl << "  Command:   " << command << endl << "  Arguments: " << args << endl << "--------------------------------" << endl;
	}
...
String ServerLootCommand::createLoot(TransactionLog& trx, CreatureObject* creature, SceneObject* container, const String& lootName, int level, float modifier) {
	StringBuffer msg;
...
TangibleObject* ServerLootCommand::createLootObject(TransactionLog& trx, CreatureObject* creature, const String& lootName, int level, float modifier) {
	auto zone = creature->getZone();
...
String ServerLootCommand::testItem(TransactionLog& trx, CreatureObject* creature, const String& args, bool createItems) {
	StringBuffer msg;
...
String ServerLootCommand::testGroup(TransactionLog& trx, CreatureObject* creature, const String& args) {
	StringBuffer msg;
	StringTokenizer tokenizer(args.toLowerCase());
...
String ServerLootCommand::searchLoot(TransactionLog& trx, CreatureObject* creature, const String& args) {
	StringBuffer msg;
	StringTokenizer tokenizer(args.toLowerCase());
...
String ServerLootCommand::agentLoot(TransactionLog& trx, CreatureObject* creature, const String& args) {
	StringBuffer msg;
	StringTokenizer tokenizer(args.toLowerCase());
	...
	msg << "AI Agent Template: " << agentTemplateString << endl
		...
		<< "Total Yellow Named Dropped: " << yellowCount << "    " << (((1.0f * yellowCount) / totalLootItems) * 100.f) << " percent" << endl
		<< endl
	...
	if (weaponCount > 0) {
		...
		if (weaponWithDotCount > 0) {
			...
			if (fireDotTotal > 0) {
				msg << "Average Fire Potency: " << (firePotency / fireDotTotal) << endl << "Average Fire Strength: " << (fireStrenth / fireDotTotal) << endl << "Average Fire Duration: " << (fireDuration / fireDotTotal) << endl << "Average Fire Use Count: " << (fireUseCount / fireDotTotal) << endl;
			}
			...
			if (poisonDotTotal > 0) {
				msg << "Average Poison Potency: " << (poisonPotency / poisonDotTotal) << endl
			...
			if (diseaseDotTotal > 0) {
				msg << "Average Disease Potency: " << (diseasePotency / diseaseDotTotal) << endl
			...
			if (bleedDotTotal > 0) {
				msg << "Average Bleed Potency: " << (bleedPotency / bleedDotTotal) << endl << "Average Bleed Strength: " << (bleedStrength / bleedDotTotal) << endl << "Average Bleed Duration: " << (bleedDuration / bleedDotTotal) << endl << "Average Bleed Use Count: " << (bleedUseCount / bleedDotTotal) << endl;
			}
		...
		msg << endl << "Total Weapons with Skill Mods: " << weaponWithSkillModCount << "    " << (((1.0f * weaponWithSkillModCount) / weaponCount) * 100.f) << " percent of weapons looted" << endl;

		if (skillModTotal > 0) {
			msg << "Average Weapon Skill Mod Value: " << (skillModsValueTotal / skillModTotal) << endl;
		}

		msg << "Weapon - One Skill Mod: " << weaponOneMod << "    " << (((1.0f * weaponOneMod) / weaponCount) * 100.f) << " percent of weapons looted" << endl
			<< "Weapon - Two Skill Mods: " << weaponTwoMods << "    " << (((1.0f * weaponTwoMods) / weaponCount) * 100.f) << " percent of weapons looted" << endl
			<< "Weapon - Three Skill Mods: " << weaponThreeMods << "    " << (((1.0f * weaponThreeMods) / weaponCount) * 100.f) << " percent of weapons looted" << endl
			<< endl;
	}
	...
	if (totalAttachments > 0) {
		msg << "Average Attachment Skill Mod Value: " << (attachmentModTotal / attachmentTotalMods) << endl
			...
			<< "Attachments - Three Skill Mods: " << attachmentThreeMods << "    " << (((1.0f * attachmentThreeMods) / totalAttachments) * 100.f) << " percent of attachments looted" << endl
			<< endl;
	}
...
String ServerLootCommand::getSyntax() {
	StringBuffer syntax;
...
void ServerLootCommand::sendSystemMessage(CreatureObject* creature, const String& systemMessage) {
	PlayerObject* ghost = creature->getPlayerObject().get();
