...
DragIncapacitatedPlayerCommand::DragIncapacitatedPlayerCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	maxMovement = 5.0;
	needsConsent = true;
}

void DragIncapacitatedPlayerCommand::getCoordinates(SceneObject* object, SceneObject* targetObject, float maxMove, WorldCoordinates* newPosition) const {
...
bool DragIncapacitatedPlayerCommand::drag(CreatureObject* player, CreatureObject* targetPlayer, float maxRange, float maxMovement, bool needsConsent) const {
...
int DragIncapacitatedPlayerCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
