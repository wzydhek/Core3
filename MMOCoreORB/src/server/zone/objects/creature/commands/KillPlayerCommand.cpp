...
KillPlayerCommand::KillPlayerCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int KillPlayerCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int KillPlayerCommand::killPlayerShip(CreatureObject* creature, const uint64& targetID, const UnicodeString& arguments) const {
	...
	if (args.hasMoreTokens()) {
		...
		if (arg.charAt(0) == '-') {
			...
			if (lowerArg == "-area" || lowerArg == "-a") {
				...
				if (range < 1) {
					StringBuffer msg;
					msg << "Invalid range given: " << arg << " " << range << endl << "Kill Player Space Command Syntax: /killPlayer [-area range]";
	...
	if (!validOption) {
		StringBuffer msg;
		msg << "Invalid Kill Player Command Arguments in Space." << endl << "Kill Player Command Space Syntax: /killPlayer [-area range]";
