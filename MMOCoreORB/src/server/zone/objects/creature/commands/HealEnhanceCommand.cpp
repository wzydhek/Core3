...
HealEnhanceCommand::HealEnhanceCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 150;
	range = 7;
}

void HealEnhanceCommand::deactivateWoundTreatment(CreatureObject* creature) const {
...
EnhancePack* HealEnhanceCommand::findEnhancePack(CreatureObject* enhancer, uint8 attribute) const {
...
bool HealEnhanceCommand::canPerformSkill(CreatureObject* enhancer, CreatureObject* patient, EnhancePack* enhancePack, int mindCostNew) const {
	if (patient->isDead())
		return false;
...
void HealEnhanceCommand::parseModifier(const String& modifier, uint8& attribute, uint64& objectId) const {
	if (!modifier.isEmpty()) {
...
void HealEnhanceCommand::sendEnhanceMessage(CreatureObject* enhancer, CreatureObject* patient, uint8 attribute, uint32 buffApplied) const {
...
uint32 HealEnhanceCommand::getEnhancePackStrength(EnhancePack* enhancePack, CreatureObject* enhancer, CreatureObject* patient) const {
	uint32 buffPower = 0;
...
uint32 HealEnhanceCommand::getBuffStrength(Buff* existingbuff, int attribute) const {
...
void HealEnhanceCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
...
void HealEnhanceCommand::doAnimations(CreatureObject* enhancer, CreatureObject* patient) const {
...
int HealEnhanceCommand::doQueueCommand(CreatureObject* enhancer, const uint64& target, const UnicodeString& arguments) const {
