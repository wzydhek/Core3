...
MaskscentCommand::MaskscentCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int MaskscentCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool MaskscentCommand::toggleMaskScent(CreatureObject* player) const {
	if (player == nullptr)
		return false;
...
bool MaskscentCommand::checkMaskScent(CreatureObject* creature) const {
