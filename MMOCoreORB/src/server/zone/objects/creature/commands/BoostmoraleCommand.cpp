...
BoostmoraleCommand::BoostmoraleCommand(const String& name, ZoneProcessServer* server) : SquadLeaderCommand(name, server) {
}

int BoostmoraleCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void BoostmoraleCommand::getWounds(CreatureObject* leader, GroupObject* group, int* wounds) const {
...
bool BoostmoraleCommand::distributeWounds(CreatureObject* leader, GroupObject* group, int* wounds) const {
	if (group == nullptr || leader == nullptr)
		return false;
