...
DumpZoneInformationCommand::DumpZoneInformationCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int DumpZoneInformationCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int DumpZoneInformationCommand::dumpGroundZone(CreatureObject* creature, Zone* zone) const {
	...
	msg << "x = " << posX << ", z = " << posZ << ", y = " << posY << ", ow = " << direction->getW() << ", ox = " << direction->getX() << ", oz = " << direction->getZ() << ", oy = " << direction->getY() << ", cellid = " << cellid << ", collisionZ = " << collisionZ;
...
int DumpZoneInformationCommand::dumpSpaceZone(CreatureObject* creature, Zone* zone) const {
	if (creature == nullptr || zone == nullptr || !zone->isSpaceZone())
		return GENERALERROR;
