...
GotoCommand::GotoCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int GotoCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
String GotoCommand::getPlanetNameByCRC(uint32 planetCRC) const {
...
float GotoCommand::calculateWaypointManhattanDistance(CreatureObject* creature, WaypointObject* wp) const {
