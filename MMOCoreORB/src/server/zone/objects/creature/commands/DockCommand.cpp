...
DockCommand::DockCommand(const String& name, ZoneProcessServer* server) : SpaceQueueCommand(name, server) {
}

int DockCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool DockCommand::getDockingPermission(ShipObject* ship, ShipObject* target) const {
...
void DockCommand::setDocking(ShipObject* ship, ShipObject* target) const {
