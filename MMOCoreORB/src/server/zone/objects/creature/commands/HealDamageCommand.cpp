...
HealDamageCommand::HealDamageCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	range = 5;
	mindCost = 50;
}

void HealDamageCommand::deactivateInjuryTreatment(CreatureObject* creature, bool isRangedStim) const {
	float modSkill = 0.0f;
...
void HealDamageCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
void HealDamageCommand::doAnimationsRange(CreatureObject* creature, CreatureObject* creatureTarget, int oid, float range) const {
	String crc;
...
StimPack* HealDamageCommand::findStimPack(CreatureObject* creature) const {
...
bool HealDamageCommand::checkTarget(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
bool HealDamageCommand::canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, StimPack* stimPack, int mindCostNew) const {
...
void HealDamageCommand::sendHealMessage(CreatureObject* creature, CreatureObject* creatureTarget, int healthDamage, int actionDamage, int mindDamage) const {
...
void HealDamageCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
	if (!creature->isPlayerCreature())
		return;
...
void HealDamageCommand::doAreaMedicActionTarget(CreatureObject* creature, CreatureObject* targetCreature, StimPack* stimPack) const {
	if (stimPack->isRangedStimPack()) {
...
void HealDamageCommand::handleArea(CreatureObject* creature, CreatureObject* areaCenter, StimPack* pharma, float range) const {
...
int HealDamageCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
