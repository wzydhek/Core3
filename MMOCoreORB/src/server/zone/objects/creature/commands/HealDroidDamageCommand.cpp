...
HealDroidDamageCommand::HealDroidDamageCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	range = 5;
	mindCost = 50;
}

void HealDroidDamageCommand::deactivateInjuryTreatment(CreatureObject* creature) const {
	int delay = 20;
...
void HealDroidDamageCommand::doAnimations(CreatureObject* creature, CreatureObject* droid) const {
	droid->playEffect("clienteffect/healing_healdamage.cef", "");

	creature->doAnimation("heal_other");
}

StimPack* HealDroidDamageCommand::findStimPack(CreatureObject* creature) const {
...
bool HealDroidDamageCommand::canPerformSkill(CreatureObject* creature, CreatureObject* droid, StimPack* stimPack, int mindCostNew) const {
...
void HealDroidDamageCommand::sendHealMessage(CreatureObject* creature, DroidObject* droid, int healthDamage, int actionDamage, int mindDamage) const {
...
int HealDroidDamageCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
