...
PlaceStructureCommand::PlaceStructureCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int PlaceStructureCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
	...
	try {
		...
		if ((distance > 100.0f) || (heightDiff > 10.f)) {
			...
			msg << "Player: " << creature->getFirstName() << " ID: " << creature->getObjectID() << " attempted invalid PlaceStructureCommand on " << zone->getZoneName() << " at " << placementLoc.toString() << " With a Player Position of: " << playerPosition.toString()
