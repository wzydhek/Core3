...
BandFlourishCommand::BandFlourishCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

bool BandFlourishCommand::doBandFlourish(CreatureObject* player, int flourishNum, String& instrumentName) {
...
int BandFlourishCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
float BandFlourishCommand::getCommandDuration(CreatureObject* object, const UnicodeString& arguments) const {
	return 0;
}