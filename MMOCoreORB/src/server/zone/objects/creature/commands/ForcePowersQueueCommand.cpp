...
ForcePowersQueueCommand::ForcePowersQueueCommand(const String& name, ZoneProcessServer* server) : CombatQueueCommand(name, server) {
	visMod = 25;
}

int ForcePowersQueueCommand::doCombatAction(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int ForcePowersQueueCommand::getFrsModifiedForceCost(CreatureObject* creature) const {
	ManagedReference<PlayerObject*> ghost = creature->getPlayerObject();
...
float ForcePowersQueueCommand::getCommandDuration(CreatureObject* object, const UnicodeString& arguments) const {
...
bool ForcePowersQueueCommand::isJediCombatQueueCommand() {
	return true;
}