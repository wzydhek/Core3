#include "PetEmoteCommand.h"
#include "server/zone/managers/creature/PetManager.h"
#include "server/zone/managers/combat/CombatManager.h"
#include "templates/params/creature/CreatureEmote.h"
#include "server/zone/managers/creature/CreatureManager.h"

PetEmoteCommand::PetEmoteCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int PetEmoteCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int PetEmoteCommand::praise(AiAgent* pet) const {
...
int PetEmoteCommand::shame(AiAgent* pet) const {
	pet->doAnimation("ashamed");
	return SUCCESS;
}

int PetEmoteCommand::alert(AiAgent* pet) const {
	pet->doAnimation("alert");
	return SUCCESS;
}

int PetEmoteCommand::summon(AiAgent* pet, PetControlDevice* controlDevice) const {
