...
TransferItemMiscCommand::TransferItemMiscCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int TransferItemMiscCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int TransferItemMiscCommand::doTransferItemMisc(CreatureObject* creature, SceneObject* objectToTransfer, SceneObject* destinationObject, int transferType, TransactionLog& trx) {
	trx.addState("transferType", transferType);
