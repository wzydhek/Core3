...
BoardShuttleCommand::BoardShuttleCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int BoardShuttleCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
	...
	for (int i = 0; i < 400; i++) {
		for (; tries > 0; --tries) {
			...
			info(true) << "\033[45;30m" << __FUNCTION__ << "():" << __LINE__ << "Destination Z mismatch: " << arrivalZone->getZoneName() << " (x:" << position.getPositionX() << ", y:" << position.getPositionY() << ", z:" << position.getPositionZ() << ")"
					   << " arrivalPointZ=" << arrivalPointZ << ", trying " << (tries - 1) << " more times.\033[0m";
		}
	}
#else
	for (; tries > 0; --tries) {
	...
	if (tries <= 0) {
		creature->error() << "BoardShuttleCommand: Failed to find suitable arrival point at " << arrivalZone->getZoneName() << " (x: " << position.getPositionX() << ", y: " << position.getPositionY() << ", z: " << position.getPositionZ() << ")"
						  << " arrivalPointZ = " << arrivalPointZ << ", using raw arrival point: " << arrivalZone->getZoneName() << " (x: " << arrivalPoint->getArrivalPositionX() << ", y: " << arrivalPoint->getArrivalPositionY() << ", z: " << arrivalPoint->getArrivalPositionZ() << ")";
...
#if DEBUG_TRAVEL
	{
		...
		info(true) << "\033[44;30m" << __FUNCTION__ << "():" << __LINE__ << " finalArrivalPoint = " << arrivalZone->getZoneName() << " (x:" << position.getPositionX() << ", y:" << position.getPositionY() << ", z:" << position.getPositionZ() << ")"
				   << " collisionZ=" << collisionZ << " arrivalPointZ = " << arrivalPointZ << "\033[0m";
	}
#endif // DEBUG_TRAVEL
...
SortedVector<ManagedReference<TicketObject*>> BoardShuttleCommand::findTicketsInInventory(CreatureObject* creature, PlanetTravelPoint* departurePoint) const {
...
void BoardShuttleCommand::sendTicketSelectionBoxTo(CreatureObject* player, SortedVector<ManagedReference<TicketObject*>> tickets) const {
...
Coordinate BoardShuttleCommand::findRandomizedArrivalPoint(CreatureObject* targetShuttleObject, PlanetManager* planetManager, Zone* arrivalZone, PlanetTravelPoint* arrivalPoint, int tries) const {
	Coordinate position;
...
#if DEBUG_TRAVEL
	{
		...
		info(true) << "\033[45;30m" << __FUNCTION__ << "():" << __LINE__ << " try#" << tries << " arrivalPoint = " << arrivalZone->getZoneName() << " (x:" << position.getPositionX() << ", y:" << position.getPositionY() << ", z:" << position.getPositionZ()
				   << ")"
					  " collisionZ = "
				   << collisionZ << "\033[0m";
	}
...
#if DEBUG_TRAVEL
		info(true) << "\033[45;30m" << __FUNCTION__ << "():" << __LINE__ << " try#" << tries << " randomized Position = " << arrivalZone->getZoneName() << " (x:" << position.getPositionX() << ", y:" << position.getPositionY() << ", z:" << position.getPositionZ()
				   << ")"
					  "\033[0m";
#endif // DEBUG_TRAVEL
	} else {
	...
	msg << " Zone: " << arrivalZone->getZoneName() << " Region: " << (region != nullptr ? region->getRegionDisplayedName() : "Null Region") << " Landing Position: (x:" << position.getPositionX() << ", y:" << position.getPositionY() << ", z:" << position.getPositionZ() << ")";

	info(true) << "\033[45;30m" << __FUNCTION__ << "():" << __LINE__ << " -- Try #" << tries << msg.toString() << "\033[0m";
