...
WaypointCommand::WaypointCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int WaypointCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool WaypointCommand::checkHasMoreTokens(CreatureObject* creature, StringTokenizer* tokenizer, float* position) const {
	if (tokenizer->hasMoreTokens()) {
...
bool WaypointCommand::getValidPosition(CreatureObject* creature, StringTokenizer* tokenizer, float* position, String* requiredArg) const {
	String arg;
...
void WaypointCommand::sendSystemMessage(CreatureObject* creature) const {
	if (advancedWaypoints) {
