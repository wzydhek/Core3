...
HealMindCommand::HealMindCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 250;
	mindWoundCost = 250;
	range = 5;
}

void HealMindCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
void HealMindCommand::sendHealMessage(CreatureObject* creature, CreatureObject* creatureTarget, int mindDamage) const {
...
int HealMindCommand::calculateWound(int wound, int poolWounds, int poolMax) const {
	int maxWound = poolMax - poolWounds - 1;
	return (Math::max(0, Math::min(maxWound, wound)));
}

int HealMindCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
