...
HealDroidWoundCommand::HealDroidWoundCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 50;
	range = 6;
}

void HealDroidWoundCommand::deactivateWoundTreatment(CreatureObject* creature) const {
	int delay = 20;
...
void HealDroidWoundCommand::doAnimations(CreatureObject* creature, CreatureObject* droid) const {
	droid->playEffect("clienteffect/healing_healwound.cef", "");

	creature->doAnimation("heal_other");
}

void HealDroidWoundCommand::sendWoundMessage(CreatureObject* creature, DroidObject* droid, uint32 woundsHealed) const {
...
bool HealDroidWoundCommand::canPerformSkill(CreatureObject* creature, CreatureObject* droid, WoundPack* woundPack, int mindCostNew) const {
...
void HealDroidWoundCommand::parseModifier(const String& modifier, uint64& objectId) const {
	if (!modifier.isEmpty()) {
...
uint8 HealDroidWoundCommand::findAttribute(CreatureObject* creature) const {
	for (int i = 0; i < 9; ++i) {
...
WoundPack* HealDroidWoundCommand::findWoundPack(CreatureObject* creature) const {
...
int HealDroidWoundCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
