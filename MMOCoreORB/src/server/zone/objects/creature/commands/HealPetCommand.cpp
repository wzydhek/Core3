...
HealPetCommand::HealPetCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	range = 5;
	mindCost = 50;
}

void HealPetCommand::deactivateInjuryTreatment(CreatureObject* creature) const {
	int delay = 20;
...
void HealPetCommand::doAnimations(CreatureObject* creature, CreatureObject* pet) const {
	pet->playEffect("clienteffect/healing_healdamage.cef", "");

	creature->doAnimation("heal_other");
}

StimPack* HealPetCommand::findStimPack(CreatureObject* creature) const {
...
bool HealPetCommand::canPerformSkill(CreatureObject* creature, CreatureObject* pet, StimPack* stimPack, int mindCostNew) const {
...
void HealPetCommand::sendHealMessage(CreatureObject* creature, CreatureObject* pet, int healthDamage, int actionDamage, int mindDamage) const {
...
int HealPetCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
