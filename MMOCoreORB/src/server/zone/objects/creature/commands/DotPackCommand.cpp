...
DotPackCommand::DotPackCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	effectName = "clienteffect/healing_healenhance.cef";
}

void DotPackCommand::doAnimationsRange(CreatureObject* creature, CreatureObject* targetCreature, uint64 oid, float range, bool area, bool isPoisonDot) const {
	StringBuffer crc;
	StringBuffer type;
...
DotPack* DotPackCommand::findDotPack(CreatureObject* creature, uint8 pool, bool poolGiven) const {
	if (creature == nullptr)
		return nullptr;
...
void DotPackCommand::parseModifier(const String& modifier, uint8& pool, uint64& objectId) const {
	if (!modifier.isEmpty()) {
...
bool DotPackCommand::checkTarget(CreatureObject* creature, CreatureObject* targetCreature, uint32 dotType) const {
...
void DotPackCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
...
void DotPackCommand::handleArea(CreatureObject* attackerCreo, CreatureObject* targetCreature, DotPack* pharma, float range) const {
...
void DotPackCommand::doAreaMedicActionTarget(CreatureObject* creature, CreatureObject* targetCreature, DotPack* dotPack) const {
...
int DotPackCommand::hasCost(CreatureObject* creature) const {
	if (!creature->isPlayerCreature())
		return 0;
...
void DotPackCommand::applyCost(CreatureObject* creature, int mindDamage) const {
	if (mindDamage == 0)
		return;
...
int DotPackCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
