...
InstallShipComponentCommand::InstallShipComponentCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int InstallShipComponentCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool InstallShipComponentCommand::isSlotCompatibleWithComponent(ShipObject* ship, ShipComponent* component, uint32 slot) const {
...
bool InstallShipComponentCommand::isPlayerCertifiedForObject(CreatureObject* player, TangibleObject* object) const {
