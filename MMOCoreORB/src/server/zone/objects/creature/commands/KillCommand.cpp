...
KillCommand::KillCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int KillCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
int KillCommand::killAiShip(CreatureObject* creature, const uint64& targetID, const UnicodeString& arguments) const {
	...
	if (args.hasMoreTokens()) {
		...
		if (arg.charAt(0) == '-') {
			...
			if (lowerArg == "-area" || lowerArg == "-a") {
				...
				if (range < 1) {
					StringBuffer msg;
					msg << "Invalid range given: " << arg << " " << range << endl << "Kill Command Space Syntax: /kill [-area range]";
	...
	if (!validOption) {
		StringBuffer msg;
		msg << "Invalid Kill Command Arguments in Space." << endl << "Kill Command Space Syntax: /kill [-area range]";
