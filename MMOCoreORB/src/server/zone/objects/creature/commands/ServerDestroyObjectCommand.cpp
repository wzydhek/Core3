...
ServerDestroyObjectCommand::ServerDestroyObjectCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int ServerDestroyObjectCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void ServerDestroyObjectCommand::destroyObject(SceneObject* object, CreatureObject* creature) const {
	if (creature != nullptr) {
