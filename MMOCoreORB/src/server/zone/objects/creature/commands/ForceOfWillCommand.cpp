...
ForceOfWillCommand::ForceOfWillCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int ForceOfWillCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void ForceOfWillCommand::doDowner(CreatureObject* player, int buffDownerValue, float duration) const {
