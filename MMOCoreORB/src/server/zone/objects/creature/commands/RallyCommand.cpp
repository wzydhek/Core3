...
RallyCommand::RallyCommand(const String& name, ZoneProcessServer* server) : SquadLeaderCommand(name, server) {
}

int RallyCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool RallyCommand::doRally(CreatureObject* leader, GroupObject* group) const {
...
void RallyCommand::sendRallyCombatSpam(CreatureObject* leader, GroupObject* group, bool success) const {
	if (leader == nullptr || group == nullptr)
		return;
