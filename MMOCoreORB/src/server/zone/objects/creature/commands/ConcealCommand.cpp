...
ConcealCommand::ConcealCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

void ConcealCommand::doAnimations(CreatureObject* creature, CreatureObject* targetPlayer) const {
...
bool ConcealCommand::checkHostileInRange(Zone* zone, CreatureObject* player) const {
...
ManagedReference<TangibleObject*> ConcealCommand::getCamoKit(Zone* zone, CreatureObject* player) const {
	if (zone == nullptr || player == nullptr)
		return nullptr;
...
int ConcealCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
