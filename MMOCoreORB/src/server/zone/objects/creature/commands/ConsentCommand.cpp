...
ConsentCommand::ConsentCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

void ConsentCommand::consent(CreatureObject* player, CreatureObject* targetPlayer) {
...
int ConsentCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
