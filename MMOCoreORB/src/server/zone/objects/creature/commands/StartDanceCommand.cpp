...
StartDanceCommand::StartDanceCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

void StartDanceCommand::startDance(CreatureObject* creature, int performanceIndex) {
...
int StartDanceCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
