...
SetFactionStandingCommand::SetFactionStandingCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

void SetFactionStandingCommand::sendInvalidParameterMessage(CreatureObject* creature) const {
	creature->sendSystemMessage("Invalid parameter. Format is /setFactionStanding <player> <faction name> <value>");
}

int SetFactionStandingCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
