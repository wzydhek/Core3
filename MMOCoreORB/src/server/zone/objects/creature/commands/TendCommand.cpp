...
TendCommand::TendCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 0;
	mindWoundCost = 0;
...
void TendCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
void TendCommand::sendHealMessage(CreatureObject* creature, CreatureObject* creatureTarget, int healthDamage, int actionDamage) const {
...
void TendCommand::sendWoundMessage(CreatureObject* creature, CreatureObject* creatureTarget, int poolAffected, int woundsHealed) const {
	if (!creature->isPlayerCreature())
		return;
...
void TendCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
	if (!creature->isPlayerCreature())
		return;
...
uint8 TendCommand::findAttribute(CreatureObject* creature, uint8 startAttribute) const {
...
int TendCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
