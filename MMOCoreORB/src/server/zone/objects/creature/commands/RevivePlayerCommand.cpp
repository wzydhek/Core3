...
RevivePlayerCommand::RevivePlayerCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 200;
	range = 7;
}

bool RevivePlayerCommand::canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, RevivePack* revivePack, int mindCostNew) const {
...
void RevivePlayerCommand::parseModifier(const String& modifier, uint64& objectId) const {
	if (!modifier.isEmpty())
...
void RevivePlayerCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
	if (!creature->isPlayerCreature())
		return;
...
void RevivePlayerCommand::applyDebuff(CreatureObject* creature) const {
...
RevivePack* RevivePlayerCommand::findRevivePack(CreatureObject* creature) const {
...
void RevivePlayerCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
int RevivePlayerCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
