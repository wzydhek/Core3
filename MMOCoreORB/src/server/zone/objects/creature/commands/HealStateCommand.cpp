...
HealStateCommand::HealStateCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 20;
	range = 6;
...
void HealStateCommand::deactivateStateTreatment(CreatureObject* creature) const {
...
void HealStateCommand::awardXp(CreatureObject* creature, String type, int power) const {
...
void HealStateCommand::sendStateMessage(CreatureObject* creature, CreatureObject* creatureTarget, uint64 state) const {
	if (!creature->isPlayerCreature())
		return;
...
void HealStateCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
bool HealStateCommand::canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, StatePack* statePack, int mindCostNew) const {
...
StatePack* HealStateCommand::findStatePack(CreatureObject* creature, uint64 state) const {
...
void HealStateCommand::parseModifier(const String& modifier, uint64& state, uint64& objectId) const {
	if (!modifier.isEmpty()) {
...
int HealStateCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
