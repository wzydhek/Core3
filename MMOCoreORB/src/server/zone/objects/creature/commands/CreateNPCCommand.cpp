...
CreateNPCCommand::CreateNPCCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int CreateNPCCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
	...
	if (arg == "tools") {
		...
		*staffTools << creature;
		...
		for (int i = 0; i < closeObjects.size(); i++) {
			...
			StringBuffer msg;
			msg << spawnEgg->getDisplayedName() << " --- Location: " << spawnEgg->getWorldPosition().toString() << " -- Spawner Set Visible. Please target with /tar spawn and use the radial option to set invisible.";
			creature->sendSystemMessage(msg.toString());
		}
	}
