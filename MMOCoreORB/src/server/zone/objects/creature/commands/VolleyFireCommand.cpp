...
VolleyFireCommand::VolleyFireCommand(const String& name, ZoneProcessServer* server) : SquadLeaderCommand(name, server) {
}

int VolleyFireCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
bool VolleyFireCommand::attemptVolleyFire(CreatureObject* player, uint64* target, int skillMod) const {
	if (player == nullptr)
		return false;
...
bool VolleyFireCommand::doVolleyFire(CreatureObject* leader, GroupObject* group, uint64* target) const {
