...
HoloEmoteCommand::HoloEmoteCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int HoloEmoteCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void HoloEmoteCommand::handleEmote(CreatureObject* player, String& arg) const {
...
void HoloEmoteCommand::handleHelp(CreatureObject* player) const {
	PlayerObject* ghost = player->getPlayerObject();
	if (ghost == nullptr)
		return;
...
void HoloEmoteCommand::handleDelete(CreatureObject* player) const {
	PlayerObject* ghost = player->getPlayerObject();
	if (ghost == nullptr)
		return;
...
bool HoloEmoteCommand::isValidArgument(const String& arg) const {
