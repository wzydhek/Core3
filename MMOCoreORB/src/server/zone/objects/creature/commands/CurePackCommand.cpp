...
CurePackCommand::CurePackCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	range = 7;
	mindCost = 100;
	state = 0;
}

void CurePackCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
uint64 CurePackCommand::parseObjectID(const String& objectIDString) const {
...
CurePack* CurePackCommand::findCurePack(CreatureObject* creature) const {
...
void CurePackCommand::sendCureMessage(CreatureObject* object, CreatureObject* target) const {
...
void CurePackCommand::deactivateConditionTreatment(CreatureObject* creature) const {
...
void CurePackCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
...
bool CurePackCommand::checkTarget(CreatureObject* creature, CreatureObject* creatureTarget) const {
	switch (state) {
		case CreatureState::POISONED:
...
void CurePackCommand::handleArea(CreatureObject* creature, CreatureObject* areaCenter, CurePack* pharma, float range) const {
...
void CurePackCommand::doAreaMedicActionTarget(CreatureObject* creature, CreatureObject* creatureTarget, PharmaceuticalObject* pharma) const {
...
bool CurePackCommand::canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, CurePack* curePack, int mindCostNew) const {
	switch (state) {
		case CreatureState::POISONED:
			if (!creatureTarget->isPoisoned()) {
				if (creature == creatureTarget)
...
int CurePackCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
