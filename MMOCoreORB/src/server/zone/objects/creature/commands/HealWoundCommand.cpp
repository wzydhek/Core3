...
HealWoundCommand::HealWoundCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
	mindCost = 50;
	range = 6;
}

void HealWoundCommand::deactivateWoundTreatment(CreatureObject* creature) const {
...
void HealWoundCommand::doAnimations(CreatureObject* creature, CreatureObject* creatureTarget) const {
...
void HealWoundCommand::awardXp(CreatureObject* creature, const String& type, int power) const {
...
void HealWoundCommand::sendWoundMessage(CreatureObject* object, CreatureObject* target, uint8 attribute, uint32 woundsHealed) const {
...
bool HealWoundCommand::canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, WoundPack* woundPack, int mindCostNew) const {
...
void HealWoundCommand::parseModifier(const String& modifier, uint8& attribute, uint64& objectId) const {
	if (!modifier.isEmpty()) {
...
uint8 HealWoundCommand::findAttribute(CreatureObject* creature, uint8 startAttribute) const {
...
WoundPack* HealWoundCommand::findWoundPack(CreatureObject* creature, uint8 attribute) const {
...
int HealWoundCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
