...
DismountCommand::DismountCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
...
int DismountCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void DismountCommand::handleMount(CreatureObject* creature, SceneObject* mount) const {
...
void DismountCommand::removeMountBuffs(CreatureObject* vehicle) const {
	if (vehicle == nullptr)
		return;
