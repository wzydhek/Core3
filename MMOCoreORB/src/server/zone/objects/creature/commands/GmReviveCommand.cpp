...
GmReviveCommand::GmReviveCommand(const String& name, ZoneProcessServer* server) : QueueCommand(name, server) {
}

int GmReviveCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void GmReviveCommand::revivePatient(CreatureObject* creature, CreatureObject* patient) const {
	Locker clocker(patient, creature);
...
void GmReviveCommand::repairShip(CreatureObject* player, SceneObject* shipSceneO) const {
