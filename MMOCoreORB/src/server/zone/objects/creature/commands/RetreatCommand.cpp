...
RetreatCommand::RetreatCommand(const String& name, ZoneProcessServer* server) : SquadLeaderCommand(name, server) {
}

bool RetreatCommand::checkRetreat(CreatureObject* creature) const {
...
int RetreatCommand::doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
...
void RetreatCommand::doRetreat(CreatureObject* player) const {
	if (player == nullptr)
		return;
