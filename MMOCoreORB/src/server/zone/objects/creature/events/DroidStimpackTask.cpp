#include "DroidStimpackTask.h"

DroidStimpackTask::DroidStimpackTask(DroidObject* droid, CreatureObject* player, String droidName) : Task() {
	this->droid = droid;
	this->target = player;
	this->droidName = droidName;
}

void DroidStimpackTask::run() {
	Locker lock(droid);
