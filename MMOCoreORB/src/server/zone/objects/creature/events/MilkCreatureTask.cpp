#include "MilkCreatureTask.h"

MilkCreatureTask::MilkCreatureTask(Creature* cre, CreatureObject* playo) : Task() {
	currentPhase = INITIAL;
	creature = cre;
	player = playo;
}

void MilkCreatureTask::run() {
...
void MilkCreatureTask::giveMilkToPlayer() {
...
void MilkCreatureTask::clearStationary() {
	creature->removeObjectFlag(ObjectFlag::STATIONARY);
	creature->setAITemplate();
}

void MilkCreatureTask::updateMilkState(const short milkState) {
