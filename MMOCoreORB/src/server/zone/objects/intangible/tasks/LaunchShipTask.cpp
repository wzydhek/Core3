#include "LaunchShipTask.h"

LaunchShipTask::LaunchShipTask(CreatureObject* creo, ShipControlDevice* controlDevice, Vector<uint64> group) : play(creo), shipDev(controlDevice), groupMembers(group) {
	setLoggingName("LaunchShipTask");
}

LaunchShipTask::LaunchShipTask(CreatureObject* creo, ShipControlDevice* controlDevice, Vector<uint64> group, String zoneName) : play(creo), shipDev(controlDevice), groupMembers(group), chosenZone(zoneName) {
	setLoggingName("LaunchShipTask");
}

void LaunchShipTask::run() {
...
void LaunchShipTask::handleDroid(CreatureObject* player, ShipObject* ship, ShipControlDevice* shipControlDevice) {
