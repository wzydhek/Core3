/*
 * ShipPermissionList.cpp
 */

#include "ShipPermissionList.h"
...
ShipPermissionList::ShipPermissionList(const ShipPermissionList& spl) : Object(), permissionLists(spl.permissionLists), idPermissionLists(spl.idPermissionLists), ownerName(spl.ownerName), ownerID(spl.ownerID), lock() {
}

bool ShipPermissionList::toBinaryStream(ObjectOutputStream* stream) {
...
ShipPermissionList& ShipPermissionList::operator=(const ShipPermissionList& list) {
	if (this == &list) {
		return *this;
	}
...
int ShipPermissionList::writeObjectMembers(ObjectOutputStream* stream) {
	String _name;
...
bool ShipPermissionList::readObjectMember(ObjectInputStream* stream, const String& name) {
...
bool ShipPermissionList::parseFromBinaryStream(ObjectInputStream* stream) {
...
void ShipPermissionList::sendTo(CreatureObject* creature, const String& listName) /*const TODO: THIS SHOULD BE CONST but there is hack modifying lists...*/ {
...
int ShipPermissionList::togglePermission(const String& listName, const uint64 objectID) {
	Locker locker(&lock);
...
int ShipPermissionList::grantPermission(const String& listName, const uint64 objectID) {
	Locker locker(&lock);
...
int ShipPermissionList::revokePermission(const String& listName, const uint64 objectID) {
	Locker locker(&lock);
...
int ShipPermissionList::revokeAllPermissions(const uint64 objectID) {
	Locker locker(&lock);
...
void ShipPermissionList::revokeAllPermissions() {
	Locker locker(&lock);
...
void ShipPermissionList::migrateLists(ZoneServer* zoneServer, uint64 ownerObjectID) {
	Locker locker(&lock);
...
bool ShipPermissionList::isOnPermissionList(const String& listName, const uint64 objectID) const {
	ReadLocker locker(&lock);
...
bool ShipPermissionList::isListFull(const String& listName) const {
	ReadLocker locker(&lock);
