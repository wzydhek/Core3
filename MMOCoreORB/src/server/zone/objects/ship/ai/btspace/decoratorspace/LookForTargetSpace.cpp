#include "LookForTargetSpace.h"
#include "templates/params/ship/ShipFlag.h"

LookForTargetSpace::LookForTargetSpace(const String& className, const uint32 id, const LuaObject& args) : DecoratorSpace(className, id, args) {
}

LookForTargetSpace::LookForTargetSpace(const LookForTargetSpace& b) : DecoratorSpace(b) {
}

BehaviorSpace::Status LookForTargetSpace::execute(ShipAiAgent* agent, unsigned int startIdx) const {
#ifdef DEBUG_SHIP_AI
...
bool LookForTargetSpace::isInvalidTarget(ShipObject* targetShip, ShipAiAgent* agent) const {
