#include "ShipAiPatrolPathFinder.h"

Vector3 ShipAiPatrolPathFinder::getRandomPosition(const Vector3& position, float radiusMin, float radiusMax) {
...
Vector<SpacePatrolPoint> ShipAiPatrolPathFinder::generatePatrolCircle(const Sphere& sphere, const Matrix4& rotation, int totalPoints) {
	...
	for (int i = 0; i < totalPoints; ++i) {
		...
		Vector3 patrolPoint = Vector3(sphereRadius * cosTheta, sphereRadius * sinTheta, 0.f);
...
Vector<SpacePatrolPoint> ShipAiPatrolPathFinder::generatePatrolSphere(const Sphere& sphere, const Matrix4& rotation, int totalPoints) {
	Vector<SpacePatrolPoint> patrolPoints;
	...
	for (int i = 0; i < circles; ++i) {
		...
		for (int ii = 0; ii < points; ++ii) {
			...
			Vector3 patrolPoint = Vector3(sphereRadius * sinPhi * cosTheta, sphereRadius * sinPhi * sinTheta, sphereRadius * cosPhi);
