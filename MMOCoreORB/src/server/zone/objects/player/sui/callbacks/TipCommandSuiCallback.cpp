#include "TipCommandSuiCallback.h"

TipCommandSuiCallback::TipCommandSuiCallback(ZoneServer* server, CreatureObject* target, int amount) : SuiCallback(server) {
	this->targetPlayer = target;
	this->amount = amount;
}

void TipCommandSuiCallback::run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
