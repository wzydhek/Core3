#include "TravelCouponUseSuiCallback.h"
#include "server/zone/managers/planet/PlanetManager.h"
#include "server/zone/objects/transaction/TransactionLog.h"

TravelCouponUseSuiCallback::TravelCouponUseSuiCallback(ZoneServer* server, SceneObject* vouch, int travelFare, String departurePl, String departurePo, String arrivalPl, String arrivalPo) : SuiCallback(server) {
	voucher = vouch;
	fare = travelFare;
...
void TravelCouponUseSuiCallback::run(CreatureObject* player, SuiBox* sui, uint32 eventIndex, Vector<UnicodeString>* args) {
	...
	if (cancelPressed) {
		...
		TransactionLog trx(player, TrxCode::TRAVELSYSTEM, static_cast<uint32>(0), false);
		trx.addState("departurePlanet", departurePlanet);
		trx.addState("departurePoint", departurePoint);
		trx.addState("arrivalPlanet", arrivalPlanet);
		trx.addState("arrivalPoint", arrivalPoint);
