#include "CloningStoreSuiCallback.h"

CloningStoreSuiCallback::CloningStoreSuiCallback(ZoneServer* server, bool coupon) : SuiCallback(server) {
	hasCoupon = coupon;
}

void CloningStoreSuiCallback::run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
