...
FriendList::FriendList(const FriendList& list) : PlayerList<7>(list) {
	reverseTable = list.reverseTable;
}

FriendList& FriendList::operator=(const FriendList& list) {
	if (this == &list) {
		return *this;
	}
...
bool FriendList::readObjectMember(ObjectInputStream* stream, const String& name) {
...
int FriendList::writeObjectMembers(ObjectOutputStream* stream) {
...
bool FriendList::toBinaryStream(ObjectOutputStream* stream) {
...
bool FriendList::parseFromBinaryStream(ObjectInputStream* stream) {
...
void FriendList::addReversePlayer(const String& name) {
...
void FriendList::removeReversePlayer(const String& name) {
	Locker locker(getLock());
...
String FriendList::getReversePlayer(int idx) {
	Locker locker(getLock());

	return reverseTable.get(idx);
}

int FriendList::reversePlayerCount() {
