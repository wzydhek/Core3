#include "SpaceAiMap.h"

SpaceBehaviorFactory ::~SpaceBehaviorFactory() {
	HashTableIterator<String, const SpaceBehaviorCreatorHelper*> it = fnMap.iterator();
	while (it.hasNext())
		delete it.next();
}

void SpaceBehaviorFactory ::reg(const String& key, const SpaceBehaviorCreatorHelper* fn) {
	if (!fnMap.containsKey(key))
		fnMap.put(key, fn);
}

BehaviorSpace* SpaceBehaviorFactory ::create(const String& key, const uint32 id, const LuaObject& args) {
...
SpaceAiMap::SpaceAiMap() : Logger("SpaceAiMap") {
	spaceAiMap.setNullValue(NULL);

	initialize();
}

SpaceAiMap::~SpaceAiMap() {
}

void SpaceAiMap::initialize() {
	if (DEBUG_MODE)
...
int SpaceAiMap::getTemplateSize() {
	return spaceAiMap.size();
}

// put the root node of a btree into this map
void SpaceAiMap::putTemplate(const String& name, Reference<BehaviorSpace*> ait) {
	spaceAiMap.put(name, ait);
}

// returns the root node of a behavior tree
BehaviorSpace* SpaceAiMap::getTemplate(const String& name) {
	if (name == "none")
		return NULL;
...
BehaviorSpace* SpaceAiMap::getTemplate(uint32 bitMask, BehaviorTreeSlotSpace treeID, uint64 customMapName) {
	if (customMapName > 0) {
...
BehaviorSpace* SpaceAiMap::createBehavior(const String& name, const uint32 id, const LuaObject& args) {
	return factory.create(name, id, args);
}

const JSONSerializationType SpaceAiMap::getStatsAsJSON() const {
...
void SpaceAiMap::registerBehaviors() {
	// special behaviors
...
void SpaceAiMap::putBitmask(Lua* lua, String key) {
...
void SpaceAiMap::loadCustomMap(Lua* lua) {
...
int SpaceAiMap::includeFile(lua_State* L) {
...
int SpaceAiMap::addSpaceAiTemplate(lua_State* L) {
