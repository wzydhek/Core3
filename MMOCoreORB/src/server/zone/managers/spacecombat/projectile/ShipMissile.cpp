#include "ShipMissile.h"

ShipMissile::ShipMissile(ShipObject* ship, uint8 weapon, uint8 projectile, uint8 component, Vector3 start, Vector3 end, float projectileSpeed, float projectileRange, float projectileRadius, uint64 miliTime)
...
bool ShipMissile::isMissile() const {
	return true;
}

// get
ManagedWeakReference<ShipObject*> ShipMissile::getTarget() const {
	return targetRef;
}

const Vector3& ShipMissile::getHardpointTranslation() const {
	return hardpointTranslate;
}

int ShipMissile::getTimeMin() const {
	return timeMin;
}

int ShipMissile::getTimeMax() const {
	return timeMax;
}

int ShipMissile::getTimeToHit() const {
	return timeToHit;
}

int ShipMissile::getDifficulty() const {
	return difficulty;
}

float ShipMissile::getServerSpeed() const {
	return serverSpeed;
}

float ShipMissile::getClientSpeed() const {
	return clientSpeed;
}

// set
void ShipMissile::setTarget(ShipObject* ship) {
	targetRef = ship;
}

void ShipMissile::setHardpointTranslation(const Vector3& value) {
	hardpointTranslate = value;
}

void ShipMissile::readMissileData(const ShipMissileData* data) {
...
void ShipMissile::calculateTimeToHit() {
...
// update
void ShipMissile::updatePosition(int deltaTime, int totalTime) {
	auto target = targetRef.get();
	if (target == nullptr) {
		return;
	}
...
Vector3 ShipMissile::getTargetPosition(ShipObject* target, float deltaTime) const {
...
void ShipMissile::debugProjectile(ShipObject* ship, int hitResult) {
	debugProjectileMessage(ship, hitResult);
	debugProjectilePath(ship);
}

void ShipMissile::debugProjectileMessage(ShipObject* ship, int hitResult) {
...
void ShipMissile::debugProjectilePath(ShipObject* ship) {
	auto target = targetRef.get();
	if (target == nullptr) {
		return;
	}
