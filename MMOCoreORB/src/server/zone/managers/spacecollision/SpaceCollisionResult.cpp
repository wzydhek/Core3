#include "SpaceCollisionResult.h"

SpaceCollisionEntry::SpaceCollisionEntry() {
	distance = FLT_MAX;
	slot = -1;
}

SpaceCollisionEntry::SpaceCollisionEntry(SceneObject* target, const ShipProjectile* projectile, const Vector3& localDirection, float intersection, int componentSlot) {
	object = target;
...
ManagedWeakReference<SceneObject*> SpaceCollisionEntry::getObject() const {
	return object;
}

const Vector3& SpaceCollisionEntry::getPosition() const {
	return position;
}

const Vector3& SpaceCollisionEntry::getDirection() const {
	return direction;
}

float SpaceCollisionEntry::getDistance() const {
	return distance;
}

bool SpaceCollisionEntry::isHitFront() const {
	return direction.getZ() >= 0.f;
}

int SpaceCollisionEntry::getSlot() const {
	return slot;
}

SpaceCollisionResult::SpaceCollisionResult() {
}

void SpaceCollisionResult::setCollision(SceneObject* target, const ShipProjectile* projectile, const Vector3& localDirection, float intersection, int componentSlot) {
	collisionMap.put(intersection, SpaceCollisionEntry(target, projectile, localDirection, intersection, componentSlot));
}

ManagedWeakReference<SceneObject*> SpaceCollisionResult::getObject(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().getObject() : nullptr;
}

const Vector3& SpaceCollisionResult::getPosition(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().getPosition() : Vector3::ZERO;
}

const Vector3& SpaceCollisionResult::getDirection(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().getDirection() : Vector3::ZERO;
}

float SpaceCollisionResult::getDistance(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().getDistance() : FLT_MAX;
}

bool SpaceCollisionResult::isHitFront(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().isHitFront() : false;
}

int SpaceCollisionResult::getSlot(int index) const {
	return collisionMap.size() > index ? collisionMap.elementAt(index).getValue().getSlot() : -1;
}

int SpaceCollisionResult::size() const {
	return collisionMap.size();
}

#ifdef SPACECOLLISION_DEBUG
void SpaceCollisionResult::debugCollision(ShipObject* ship, const ShipProjectile* projectile) {
	...
	StringBuffer msg;
	msg << "  distance:       " << getDistance() << endl << "  position:       " << getPosition().toString() << endl << "  front/back:     " << (isHitFront() ? "FRONT" : "BACK") << endl << "  slot:           " << getSlot() << endl;
...
String SpaceCollisionResult::debugCollisionHardpoints(ShipObject* targetShip, const Vector3& targetPosition, const Matrix4& targetRotation, const ShipCollisionData* targetData, CreateClientPathMessage* path) {
	StringBuffer msg;
