#include "DroidMechanics.h"

float DroidMechanics::determineHam(float quality, int droidType) {
	int maxHam = 55;
	int minHam = 45;
...
float DroidMechanics::determineSpeed(int droidType, float ham) {
	if (droidType == DroidObject::R_SERIES)
...
float DroidMechanics::determineHit(int droidType, float ham) {
	if (droidType == DroidObject::R_SERIES)
...
float DroidMechanics::determineMinDamage(int droidType, int rating) {
	if (rating == 0)
		return 1;
	if (droidType == DroidObject::R_SERIES)
...
float DroidMechanics::determineMaxDamage(int droidType, int rating) {
	if (rating == 0)
		return 1;
	if (droidType == DroidObject::R_SERIES)
...
int DroidMechanics::determineDroidSkillBonus(float playerSkill, float droidSkill, float baseAmount) {
