...
float Genetics::physiqueFormula(float physique, float prowess, float mental, float psychology, float aggression) {
...
float Genetics::prowessFormula(float physique, float prowess, float mental, float psychology, float aggression) {
	float result = (physique * 0.25f) + (prowess * 0.42f) + (mental * 0.17f) + (psychology * 0.085f) + (aggression * 0.075f);
	return result > 1000 ? 1000.0f : result < 0 ? 0.f : result;
}

// Intellect and Cleverness
float Genetics::mentalFormula(float physique, float prowess, float mental, float psychology, float aggression) {
	float result = (physique * 0.05f) + (prowess * 0.1f) + (mental * 0.5f) + (psychology * 0.3f) + (aggression * 0.05f);
	return result > 1000 ? 1000.0f : result < 0 ? 0.f : result;
}

// Dependability and Courage
float Genetics::physchologicalFormula(float physique, float prowess, float mental, float psychology, float aggression) {
	float result = (physique * 0.09f) + (prowess * 0.05f) + (mental * 0.26f) + (psychology * 0.43f) + (aggression * 0.17f);
	return result > 1000 ? 1000.0f : result < 0 ? 0.f : result;
}

// Fierceness and Power
float Genetics::aggressionFormula(float physique, float prowess, float mental, float psychology, float aggression) {
 ...
 */
bool Genetics::hasSpecialResist(DnaComponent* componentA, DnaComponent* componentB, DnaComponent* componentC, DnaComponent* componentD, DnaComponent* componentE, int type) {
 ...
 */
bool Genetics::hasVulnerability(float a, float b, float c, float d, float e) {
...
*/
float Genetics::getProperResistance(float value, bool special, bool override) {
 ...
 */
float Genetics::resistanceFormula(DnaComponent* componentA, DnaComponent* componentB, DnaComponent* componentC, DnaComponent* componentD, DnaComponent* componentE, int type, int max) {
#ifdef DEBUG_GENETIC_LAB
...
float Genetics::initialValue(float maxValue) {
...
float Genetics::resistanceToValue(float effective, int armor, int quality) {
...
int Genetics::randomizeValue(int value, int quality) {
...
int Genetics::calculatePetLevel(GeneticComponent* pet) {
#ifdef DEBUG_GENETIC_LAB
...
int Genetics::calculateAgentLevel(int health, float dps, float hit, int regen, int armor, float effective, float kin, float eng, float bla, float heat, float cold, float elec, float acid, float stun) {
...
int Genetics::calculateArmorValue(GeneticComponent* pet, int armorLevel, int baseLevel, int armorBase) {
	// Armor Level
...
int Genetics::resistMath(int input, int rating, int effectiveness, bool multiply, int multValue, int lowValue) {
...
float Genetics::determineMaxExperimentation(float min, float max) {
...
// level factor rules
float Genetics::calculateArmorLevel(float armorBase) {
	return DnaManager::instance()->levelForScore(DnaManager::ARM_LEVEL, armorBase);
}

// dps to level range
float Genetics::calculateDPSLevel(float maxDamage, float minDamage) {
	float dps = (maxDamage + minDamage) / 2;
	return DnaManager::instance()->levelForScore(DnaManager::DPS_LEVEL, dps);
}

// ham level
float Genetics::calculateHamLevel(float ham) {
	return DnaManager::instance()->levelForScore(DnaManager::HAM_LEVEL, ham);
}

// regen level
float Genetics::calculatRegenLevel(float regen) {
	return DnaManager::instance()->levelForScore(DnaManager::REG_LEVEL, regen);
}

// hit level
float Genetics::calculatHitLevel(float toHit) {
	return DnaManager::instance()->levelForScore(DnaManager::HIT_LEVEL, toHit);
}

int Genetics::calcArmorLevelByStats(int armorRating, int armorLevel, int baseLevel, int armorBase, float kinetic, float energy, float blast, float heat, float cold, float electricity, float acid, float stun) {
	int level = armorLevel + 1;
	if (level < baseLevel)
		level = baseLevel;
	int eff = armorBase / 50;
