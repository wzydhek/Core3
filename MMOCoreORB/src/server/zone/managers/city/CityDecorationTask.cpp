#include "CityDecorationTask.h"
#include "server/zone/managers/city/CityManager.h"

CityDecorationTask::CityDecorationTask(CreatureObject* creature, TangibleObject* object, byte function) {
	mayor = creature;
	obj = object;
	option = function;
}

void CityDecorationTask::run() {
	switch (option) {
		case 0:
			placeDecoration();
			break;
...
void CityDecorationTask::placeDecoration() {
	Locker _lock(mayor);
...
void CityDecorationTask::removeDecoration() {
	Locker _lock(mayor);
