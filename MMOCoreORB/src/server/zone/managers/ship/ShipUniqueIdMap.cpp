...
uint16 ShipUniqueIdMap::getUniqueID() {
	Locker mLock(&mutex);
...
uint16 ShipUniqueIdMap::setUniqueID(ShipObject* ship) {
	if (ship == nullptr) {
		return 0;
	}
...
void ShipUniqueIdMap::dropUniqueID(ShipObject* ship) {
	if (ship == nullptr) {
		return;
	}
...
void ShipUniqueIdMap::safeCopyTo(Vector<ManagedWeakReference<ShipObject*>>& vector) const {
	Locker mLock(&mutex);
...
int ShipUniqueIdMap::size() const {
	Locker mLock(&mutex);
