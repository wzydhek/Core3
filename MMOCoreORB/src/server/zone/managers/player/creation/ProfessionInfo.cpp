...
SkillInfo::SkillInfo(const SkillInfo& pi) : Object(), TemplateData(), IffTemplate() {
	skills = pi.skills;
	items = pi.items;
	attributeMods = pi.attributeMods;
}

SkillInfo& SkillInfo::operator=(const SkillInfo& pi) {
	if (this == &pi)
		return *this;
...
void SkillInfo::addItem(const String& templateName, const String& itemName) {
...
void SkillInfo::setAttributeMod(int idx, int value) {
	attributeMods.set(idx, value);
}

int SkillInfo::getAttributeMod(int idx) {
	return attributeMods.get(idx);
}

void SkillInfo::readObject(IffStream* iffStream) {
