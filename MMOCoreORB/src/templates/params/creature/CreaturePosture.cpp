...
CreatureMovementEntry::CreatureMovementEntry() : Variable() {
	posture = 0;
	stationary = 0;
	slow = 0;
	fast = 0;
...
CreatureMovementEntry::CreatureMovementEntry(const CreatureMovementEntry& m) : Variable() {
	posture = m.posture;
...
CreatureMovementEntry& CreatureMovementEntry::operator=(const CreatureMovementEntry& m) {
	if (this == &m) {
		return *this;
	}
...
bool CreatureMovementEntry::operator==(CreatureMovementEntry entry) {
	return posture == entry.posture; // only need posture here because I am sure that these are unique to entries
}

bool CreatureMovementEntry::toString(String& str) const {
...
bool CreatureMovementEntry::parseFromString(const String& str, int version) {
...
bool CreatureMovementEntry::toBinaryStream(ObjectOutputStream* stream) {
...
bool CreatureMovementEntry::parseFromBinaryStream(ObjectInputStream* stream) {
...
CreaturePosture::CreaturePosture() {
...
const String CreaturePosture::postureToString(uint8 posture) {
	switch (posture) {
		case INVALID:
			return "invalid";
		case UPRIGHT:
...
uint8 CreaturePosture::getLocomotion(uint8 pos, uint8 speed) const {
...
uint8 CreaturePosture::getSpeed(uint8 pos, uint8 loc) const {
	const CreatureMovementEntry* move = &movementTable.get(pos);
...
float CreaturePosture::getMovementScale(uint8 pos) const {
	return movementTable.get(pos).movementScale;
}

float CreaturePosture::getAccelerationScale(uint8 pos) const {
	return movementTable.get(pos).accelerationScale;
}

float CreaturePosture::getTurnScale(uint8 pos) const {
	return movementTable.get(pos).turnScale;
}

float CreaturePosture::getCanSeeHeightMod(uint8 pos) const {
	return movementTable.get(pos).canSeeHeightMod;
}

void CreaturePosture::loadMovementData() {
...
int CreaturePosture::getRangedAttackMod(uint8 loc) const {
	return rangedAttackMod.get(loc);
}

int CreaturePosture::getRangedDefenseMod(uint8 loc) const {
	return rangedDefenseMod.get(loc);
}

int CreaturePosture::getMeleeAttackMod(uint8 loc) const {
	return meleeAttackMod.get(loc);
}

int CreaturePosture::getMeleeDefenseMod(uint8 loc) const {
