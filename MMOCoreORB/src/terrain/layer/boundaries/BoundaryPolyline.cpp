...
BoundaryPolyline::BoundaryPolyline() : Boundary('BPLN'), lineWidth(0) {
...
void BoundaryPolyline::parseFromIffStream(engine::util::IffStream* iffStream) {
...
float BoundaryPolyline::checkInfluence(float x, float y) const {
	return 0;
}

void BoundaryPolyline::parseFromIffStream(engine::util::IffStream* iffStream, Version<'0001'>) {
...
void BoundaryPolyline::translateBoundary(float x, float y) {
	minX = 800000000;
	minY = 800000000;
...
void BoundaryPolyline::initialize() {
	for (int i = 0; i < points.size(); ++i) {
		Point2D* point = points.get(i);
...
bool BoundaryPolyline::containsPoint(float x, float y) const {
	return process(x, y) != 0;
}

float BoundaryPolyline::process(float x, float y) const {
	if (x < minX)
		return 0.0;
...
float BoundaryPolyline::getMinX() const {
	return minX;
}

float BoundaryPolyline::getMaxX() const {
	return maxX;
}

float BoundaryPolyline::getMinY() const {
	return minY;
}

float BoundaryPolyline::getMaxY() const {
	return maxY;
}