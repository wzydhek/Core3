...
const Vector<Point2D*>& BoundaryPolygon::getVertices() const {
	return vertices;
}

BoundaryPolygon::BoundaryPolygon() : Boundary('BPOL'), localWaterTableEnabled(0), localWaterTableHeight(0), shaderSize(0) {
...
void BoundaryPolygon::executeRule(ProceduralTerrainAppearance* generator) {
	if (localWaterTableEnabled != 0)
		generator->insertWaterBoundary(this);
}

void BoundaryPolygon::translateBoundary(float x, float y) {
	minX = 800000000;
	minY = 800000000;
...
void BoundaryPolygon::initialize() {
	for (int i = 0; i < vertices.size(); ++i) {
...
float BoundaryPolygon::checkInfluence(float x, float y) const {
...
float BoundaryPolygon::process(float x, float y) const {
	return checkInfluence(x, y);
}

bool BoundaryPolygon::containsPoint(float px, float py) const {
...
void BoundaryPolygon::parseFromIffStream(engine::util::IffStream* iffStream) {
...
void BoundaryPolygon::parseFromIffStream(engine::util::IffStream* iffStream, Version<'0005'>) {
...
float BoundaryPolygon::getLocalWaterTableHeight() const {
	return localWaterTableHeight;
}

float BoundaryPolygon::getMinX() const {
	return minX;
}

float BoundaryPolygon::getMaxX() const {
	return maxX;
}

float BoundaryPolygon::getMinY() const {
	return minY;
}

float BoundaryPolygon::getMaxY() const {
	return maxY;
}