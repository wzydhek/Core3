...
void TreeArchive::unpackFile(const String& file) {
	TreeFile treeFile(this);
	treeFile.read(file);
}

void TreeArchive::addRecord(const String& path, TreeFileRecord* record) {
	try {
...
const TreeDirectory* TreeArchive::getTreeDirectory(const String& path) const {
 ...
 */
byte* TreeArchive::getBytes(const String& recordPath, int& size) const {
...
const TreeDirectory* TreeArchive::getDirectory(const String& path) const {
	return nodeMap.get(path);
}

Vector<String>* TreeArchive::getFilesAndSubDirectoryFiles(const String& directory) const {
